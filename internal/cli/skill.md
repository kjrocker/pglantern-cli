...
- `annual quota exceeded` / `daily quota exceeded` / `burst rate limit exceeded`
  ...
  with a key to raise the limits. With a key, the cap is your **account's**,
  shared across every key you hold, so another machine may have spent it;
  `/users/api-keys` shows which, and the band there says when the pool refills.
  The budget is in cost units — a search costs 5, a listing 2, a lookup 1 — so a
  search can be refused while cheaper commands still work. On an annual 429 the
  `retry-after` is measured in months, so treat it as "get more quota", not as a
  sleep interval; `x-ratelimit-reset` carries the exact reset instant.
